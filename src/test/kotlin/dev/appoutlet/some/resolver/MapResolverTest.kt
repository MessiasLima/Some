...
class MapResolverTest {
    ...
    @Test
    fun `MapResolver throws error on star projection in key`() {
        val resolver = MapResolver(DefaultStrategyProvider(), Random.Default)

        assertFailsWith<IllegalStateException> {
            resolver.resolve(typeOf<Map<*, String>>(), defaultTestChain)
        }
    }

    @Test
    fun `MapResolver throws error on star projection in value`() {
        val resolver = MapResolver(DefaultStrategyProvider(), Random.Default)
